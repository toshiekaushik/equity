...
class Cross_Sectional_Regression(Factors):
    ...
    def execute(self):
        ...
        volatility = self.factor_pg.get_volatility_20_days("2023-01-01", "2026-08-01", self.tickers)
        ...
        volatility = volatility.sort_values(["ticker", "date"])
        df = momentum.merge(
        ...
        df = df.merge(
            volatility[["ticker", "date", "volatility"]]
        )
    ...
    def cross_sectional_regression(self, data: DataFrame):
        ...
        for date, cross_section in data.groupby("date"):
            reversal_coeff, mom_4_1_coef, mom_12_1_coeff, const_coeff, vol_coeff= self.regression(cross_section)
        ...
        vol_returns = [val for val in vol_returns if not math.isnan(val)]
        ...
        vol_avg = np.nansum(vol_returns) / len(vol_returns)
        ...
        print("Average volatility factor return: ", vol_avg)
        ...
        self.time_returns_pg.upsert_df_columns(alpha_model, "first_alpha_model")

    def regression(self, cross_section: DataFrame) -> tuple:
        X = cross_section[["mom_4_1", "mom_12_1", "reversal_chg", "volatility"]]
        ...
        return (model.params["reversal_chg"],
                model.params["mom_4_1"],
                model.params["mom_12_1"],
                model.params["const"],
                model.params["volatility"])
